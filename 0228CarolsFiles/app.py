...
def school_ranking_all():
    ...
    school_data = school_df.loc[:,["Name", "Ranking", "Address","Median_Price"]]
...
@app.route("/properties_by_school/<school>")
def properties_by_school(school):
    ...
    results = db.session.query(*sel).filter(Property_Details.school == school).all()
